...
public class MyFancyCoverFlowAdapter1 extends FancyCoverFlowAdapter {
    ...
    public MyFancyCoverFlowAdapter1(Context context, List<Item> list) {
        mContext = context;
        this.list = list;
    }
    ...
    public View getCoverFlowItem(int position, View convertView, ViewGroup parent) {
        ViewHolder holder;
        if (convertView == null) {
            convertView = LayoutInflater.from(mContext).inflate(R.layout.item_fancycoverflow1, null);
        ...
        // TODO: 特别注意：滑动当前控件(设置了OnClickListener)无法实现滚动效果
